...
def _parse_jsonl_line(line: str, session_id: str, agent_id: str = None) -> Optional[dict]:
    ...
    if record_type == "session":
        ...
        agent_id = None
        if cwd:
            # 标准路径：/agents/{agent_name}/
            match = re.search(r'/agents/([^/]+)/', cwd)
            if match:
                agent_id = match.group(1)
            else:
                # V7.8-9-4-3: 兜底——默认 workspace 路径识别为 main
                if '/.openclaw/workspace' in cwd or '/openclaw/workspace' in cwd:
                    agent_id = 'main'
                # V7.8-9-4-3 FIX: 额外兜底——任何包含 .openclaw 但不含 /agents/ 的路径都归为 main
                elif '.openclaw' in cwd and '/agents/' not in cwd:
                    agent_id = 'main'
        return {
...
def _get_session_id_from_path(jsonl_path: Path) -> str:
    """
    从 JSONL 文件路径提取 import_source_id（数据源标识符）
    
    ⚠️ 语义说明：
    - 返回值实际是 **import_source_id**（数据源/文件名），不是传统意义上的"会话ID"
    - 一个文件可能包含多个时间上不连续的对话（如全天 08:00~23:00 的消息）
    - 真正的"会话"由 BatchSessionCutter 后续切割生成（格式: {source}#{segment_index}）
    
    命名历史原因：
    - 字段名为 session_id 是为了向下兼容
    - 在代码注释和文档中应理解为 "import_source_id" 或 "conversation_group_id"
    
    Args:
        jsonl_path: JSONL 文件路径，如 "/path/to/c8584676.jsonl"
    
    Returns:
        文件名字符串，如 "c8584676"
    
    Example:
        >>> _get_session_id_from_path(Path("c8584676.jsonl"))
        'c8584676'
        >>> _get_session_id_from_path(Path("abc123.reset.jsonl"))
        'abc123'
    """
    return jsonl_path.stem.split(".jsonl")[0].split(".reset")[0].split(".checkpoint")[0]
...
def collect_session(jsonl_path: Path, db_path: Path, batch_size: int = 50) -> int:
    ...
    # V7.8-9-4-3: 从文件路径预提取 agent_id（兜底机制）
    # 解决 cwd 格式不标准（如 ~/.openclaw/workspace）导致正则匹配失败
    path_match = re.search(r'/agents/([^/]+)/sessions/', str(jsonl_path))
    fallback_agent_id = path_match.group(1) if path_match else None

    # 如果路径中没有 /agents/，尝试识别默认 workspace
    if not fallback_agent_id:
        if '/.openclaw/workspace' in str(jsonl_path) or '/openclaw/workspace' in str(jsonl_path):
            fallback_agent_id = 'main'

    agent_id = fallback_agent_id  # ← 用路径预提取的值作为初始值
...
def collect_all_sessions(jsonl_dir: Path, db_path: Path, batch_size: int = 50) -> int:
    # 严格过滤：只保留标准 .jsonl 文件，排除 .reset.* 和 .checkpoint.* 备份
    jsonl_files = [
        f for f in jsonl_dir.iterdir()
...
def run_async_collector(jsonl_dir: Optional[Path] = None, db_path: Path = Path("data/flow_ecosystem.db"),
                        poll_interval: int = 30, batch_size: int = 50):
    """
    异步采集器 - 持续监控并采集新数据

    参数:
        jsonl_dir: JSONL文件目录，为None时自动发现
        db_path: 数据库路径
        poll_interval: 轮询间隔（秒），默认30秒
        batch_size: 批量大小
    """
    if jsonl_dir is None:
        from core.openclaw_path_resolver import find_openclaw_data_dir
        openclaw_dir = find_openclaw_data_dir()
        if not openclaw_dir:
            raise RuntimeError(
                "未找到 OpenClaw 数据目录。请设置环境变量 OPENCLAW_DATA_DIR "
                "或创建 ~/.flow_evolution/config 指定路径"
            )
        jsonl_dir = openclaw_dir / "agents"
...
def run_session_end_collector(jsonl_dir: Optional[Path] = None, db_path: Path = Path("data/flow_ecosystem.db"),
